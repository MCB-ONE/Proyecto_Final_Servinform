...
using Core.Specification.Factura;
...
namespace WebApi.Controllers
{
    public class FacturaController : BaseApiController
    {
        ...
        [HttpPost]
        [Authorize]
        public async Task<ActionResult<FacturaDto>> CreateFactura(CreateFacturaDto facturaCreate)
        {
            var emailUsuario = HttpContext.User?.Claims?.FirstOrDefault(x => x.Type == ClaimTypes.Email)?.Value;

            var lineasFactura = _mapper.Map<List<LineaFactura>>(facturaCreate.LineasFactura);
        ...
        [Authorize]
        public async Task<ActionResult<List<FacturaDto>>> UpdateFactura(int id, CreateFacturaDto facturaUpdated)
        {
